...
class LMIM(nn.Module):
    ...
    """
    def __init__(self, grid_size=14, patch_size=16, patch_gap=0, in_chans=1,
                 ...
                 avg_sim_coeff=0., gradient_directions=30, mask_ratio=0.25,mask_target=True,
        ...
        self.target_encoder = VIT(patchify=self.patchify, embed_layer=embed,
                                  grid_size=grid_size, embed_dim=embed_dim, num_heads=num_heads, depth=12,
                                  mlp_ratio=mlp_ratio, norm_layer=norm_layer, head=head, mask=False, cls=False,
                                  drop=drop, attn_drop=attn_drop, drop_path=drop_path, freeze_pe=freeze_pe)
    ...
    @torch.no_grad()
    def create_views(self, patch_pix, mask_ratio):
        N, L, D = patch_pix.shape

        num_patches_per_frame = self.grid_size[0] * self.grid_size[1] * self.grid_size[2]  # 14x14
        # total_patches = gradient_directions * num_patches_per_frame
        num_masks_per_frame = int(mask_ratio * num_patches_per_frame)

        mask_per_frame = torch.concat([
            torch.zeros(num_patches_per_frame - num_masks_per_frame),
            torch.ones(num_masks_per_frame),
        ])

        random_index = torch.randperm(mask_per_frame.numel())

        mask_per_frame = mask_per_frame[random_index]
        shuffle = mask_per_frame.repeat(self.gradient_directions, 1).flatten(0)

        vis_idx = torch.where(shuffle == 0)[0].repeat(N, 1).to(device=patch_pix.device)      # idx of vis patches
        mask_idx = torch.where(shuffle > 0)[0].repeat(N, 1).to(device=patch_pix.device)

        if self.mask_target:
            trg_idx = mask_idx
        else:
            trg_idx = torch.concat([vis_idx,mask_idx])
    ...
    def forward(self, imgs, mom=0.99, sim_trg=0.75,  update_ema=False):
        ...
        pix_vis, pid_vis, pix_trg, pid_trg, mask_idx = self.create_views(patch_pix, self.mask_ratio)
        # Compute targets
        with torch.no_grad():
            if update_ema:
        ...
        if self.avg_sim_coeff > 0:
            ...
            loss += self.avg_sim_coeff * (avg_pairwise_sim(x_vis, x_vis) - sim_trg).pow(2)
        # Log metrics
        metrics = {
            'avg_sim_pred': avg_pairwise_sim(x_pred, x_pred).item(),
            'avg_sim_vis': avg_pairwise_sim(x_vis, x_vis).item(),
...
def build_lmim(backbone, decoder_depth=3, decoder_embed_dim=384, decoder_num_heads=16, **kwargs):
    cfg = CFG[backbone]
    model = LMIM(
