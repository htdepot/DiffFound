...
from timm.models.layers import to_2tuple, to_3tuple, Mlp, DropPath
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=16, in_chans=1, embed_dim=768):
        ...
        self.proj = nn.Linear(in_chans * patch_size[0] * patch_size[1] * patch_size[2], embed_dim)
...
class Patchify:
    ...
    def __init__(self, patch_size, grid_size=None, gradient_directions=None):
        self.patch_size = to_3tuple(patch_size)
        self.grid_size = to_3tuple(grid_size)
        self.gradient_directions = gradient_directions
        self.num_patches = self.grid_size[0] * self.grid_size[1] * self.grid_size[2] * self.gradient_directions
    ...
    def patchify(self, imgs, gap):
        ...
        """
        bs = imgs.shape[0]
        ph, pw, pl = self.patch_size
        h, w, l = imgs.shape[-3:]
        assert w % (pw + gap) == 0 and h % (ph + gap) == 0 and l % (pl + gap) == 0

        th, tw, tl = h // (ph + gap), w // (pw + gap), l // (pl + gap)
        x = imgs.reshape(shape=(bs, self.gradient_directions, th, ph + gap, tw, pw + gap, tl, pl + gap))
        x = torch.einsum('nchpwqlo->nchwlpqo', x)
        if gap > 0:
        ...
        x = x.reshape(shape=(bs, self.gradient_directions * th * tw * tl, ph * pw * pl))
        return x
...
class build_3d_sincos_position_embedding(nn.Module):

    def __init__(self, grid_size, embed_dim, num_tokens=0, temperature=10000.):
        super().__init__()
        ...
        pos_emb = torch.cat([torch.sin(out_h), torch.cos(out_h), torch.sin(out_w), torch.cos(out_w), torch.sin(out_d),
                             torch.cos(out_d)], dim=1)[None, :, :]
        pos_emb = pos_emb.repeat(1, 30, 1, 1).reshape([1, -1, embed_dim])
        # if cls_token:
        #     pos_emb = np.concatenate([np.zeros([1, 1, embed_dim]), pos_emb], axis=0)
        assert num_tokens == 1 or num_tokens == 0, "Number of tokens must be of 0 or 1"
        if num_tokens == 1:
            pe_token = torch.zeros([1, 1, embed_dim], dtype=torch.float32)
            self.pos_embeds = nn.Parameter(torch.cat([pe_token, pos_emb], dim=1))
        else:
            self.pos_embeds = nn.Parameter(pos_emb)
        self.pos_embeds.requires_grad = False

    def forward(self, pid, cls=False):
        # N = pid.shape[0]
        # pos_embeds = self.pos_embeds
        if cls:
            pid = pid + 1  # pid doesn't include cls token
            pid = F.pad(pid, (1, 0, 0, 0), value=0)
        return self.pos_embeds[0][pid]
...
class VIT(nn.Module):
    def __init__(self, grid_size=(4, 4), embed_dim=1024, depth=24, num_heads=16, mlp_ratio=4.,
                 embed_layer=None, patchify=None, head=None, norm_layer=nn.LayerNorm,
                 drop=0., attn_drop=0., drop_path=0., cls=False, mask=False, freeze_pe=True,
                 avg_vis_mask_token=True, ):
        super().__init__()

        grid_size = to_3tuple(grid_size)
        self.patchify = patchify
        self.embed = PatchEmbed(patch_size=16, in_chans=1, embed_dim=embed_dim)
        self.grid_size = grid_size
        self.embed_dim = embed_dim

        self.pos_embed = build_3d_sincos_position_embedding(grid_size, embed_dim)
    ...
    def forward(self, x, pid=None, mask_pid=None):
        ...
        # Embed patches  this is need to be update
        x = self.embed(x)
