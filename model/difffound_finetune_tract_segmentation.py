...
class DiffFound(nn.Module):
    ...
    def __init__(self, grid_size=14, patch_size=16, patch_gap=0, in_chans=30,
        ...
        nn.Linear(embed_dim, embed_dim // down_radio),
        nn.GELU(),
        nn.Linear(embed_dim // down_radio, embed_dim // down_radio),
        nn.GELU()
        )
        ...
        self.out = Ps_Conv(in_channels=128, out_channels=72)
    ...
    def forward(self, imgs):
        ...
        out = dec0 + convBlock
        logit = self.out(out).permute(0, 2, 3, 4, 1).contiguous()
