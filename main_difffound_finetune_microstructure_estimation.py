...
def main_worker(local_rank, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.eval_freq == 0 or epoch == 0:
            test_stats = evaluate(data_loader_eval, model, device, epoch, args.mask_data_path, args.mask_name, args.log.rank, args.log.use_wandb)
...
def train_one_epoch(model: torch.nn.Module,
    ...
    for it, batch in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.cuda.amp.autocast():
            ...
            loss = criterion(outputs, targets.unsqueeze(4))
...
def evaluate(data_loader, model, device):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        vis_output, vis_target, nrmse_all, psnr_all, ssim_all  = (
            calculating_val_metrics_onesubject_gfa(out, target.numpy(), mask_path, save_path=False))
