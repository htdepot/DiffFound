...
def evaluate(data_loader, model, device):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        acc_all = calculating_val_tract_metrics_onesubject(out, target.numpy(), mask_path, save_image=False)
    ...
    print('Dice avg {acc:.3f} {acc_std:.3f}'
          .format(acc=np.array(dice_sall).mean(), acc_std=np.array(dice_sall).std()))
